...
export default function PrivateChat() {
    ...
    const chatMessages = useSelector(state => state && state.private);
    const otherUser = useSelector(state => state && state.other);
    ...
    useEffect(() => {
        console.log("private chat hooks component has MOUNTED");
        socket.emit('otherUserId', otherUser.id);
    ...
    const keyCheck = e => {
        if (e.key === 'Enter') {
            ...
            socket.emit('my new private chat message', e.target.value);
